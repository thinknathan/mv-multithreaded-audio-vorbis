 ...
 */
import Module from '/js/plugins/variable-buffer-kernel.wasmmodule.js';
import {
    HeapAudioBuffer,
    RingBuffer
} from '/js/plugins/wasm-audio-helper.js';
...
class AudioCopyProcessor extends AudioWorkletProcessor {
    ...
    process(inputList, outputList, parameters) {
        ...
        if (this._decodedStream && this._decodedStream.length) {
            
            var thatb = [this._decodedStream.pop()];

            this._inputRingBuffer.push(thatb);
            ...
            this._outputRingBuffer.pull(outputList[0]);
    ...
    handleStream(result) {
        if (result.data)
            this._decodedStream.push(result.data[0]);
    }

    arrayBufferToHeap(buffer, byteOffset, byteLength) {
        var ptr = this.stbvorbis._malloc(byteLength);
        var heapBytes = new Uint8Array(this.stbvorbis.HEAPU8.buffer, ptr, byteLength);
        heapBytes.set(new Uint8Array(buffer, byteOffset, byteLength));
        return heapBytes
    }
    ptrToInt32(ptr) {
        var a = new Int32Array(this.stbvorbis.HEAPU8.buffer, ptr, 1);
        return a[0]
    }
    ptrToFloat32(ptr) {
        var a = new Float32Array(this.stbvorbis.HEAPU8.buffer, ptr, 1);
        return a[0]
    }
    ptrToInt32s(ptr, length) {
        var buf = new ArrayBuffer(length * Int32Array.BYTES_PER_ELEMENT);
        var copied = new Int32Array(buf);
        copied.set(new Int32Array(this.stbvorbis.HEAPU8.buffer, ptr, length));
        return copied
    }
    ptrToFloat32s(ptr, length) {
        var buf = new ArrayBuffer(length * Float32Array.BYTES_PER_ELEMENT);
        var copied = new Float32Array(buf);
        copied.set(new Float32Array(this.stbvorbis.HEAPU8.buffer, ptr, length));
        return copied
    }
    ...
    createStbvorbis() {
        ...
        if (ENVIRONMENT_IS_NODE) {
            ...
            if (ENVIRONMENT_IS_WEB) {
                ...
                scriptDirectory = window.location.href
            }
        ...
        var _stb_vorbis_js_channels = Module["_stb_vorbis_js_channels"] = function () {
        ...
        var _stb_vorbis_js_sample_rate = Module["_stb_vorbis_js_sample_rate"] = function () {
            return Module["asm"]["_stb_vorbis_js_sample_rate"].apply(null, arguments)
        };
